...
object LoyaltyActor {
  ...
  case class ApplyLoyaltyAdjustment(adjustment: LoyaltyAdjustment)
      extends Command
  case class LoyaltyAdjustmentApplied(adjustment: LoyaltyAdjustment)
      extends Event
  case class LoyaltyAdjustmentRejected(adjustment: LoyaltyAdjustment,
                                       reason: String)
      extends Event
...
class LoyaltyActor(loyaltyRepository: LoyaltyRepository)
  ...
  private def handle(cmd: ApplyLoyaltyAdjustment) = {
    cmd match {
      case ApplyLoyaltyAdjustment(Deduct(points))
          if points > loyaltyInformation.currentTotal =>
        log.info(s"Insufficient Points For ${loyaltyId.value}")
        sender() ! LoyaltyAdjustmentRejected(
          Deduct(points),
          "Insufficient Points"
        )
      case ApplyLoyaltyAdjustment(adjustment) =>
        ...
        loyaltyRepository
          .updateLoyalty(loyaltyId, loyaltyInformation)
          .map { _ =>
